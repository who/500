...
contain 581 redeals, 108 NULLA and 36 DNULLA contracts, 10 slams, and one
joker led to a no-trump-type trick (a `named_suit` record) — every edge case
called out by the issue packet. (The NULLA/DNULLA split shifted with the
double-nulla precondition, fh-17b: double nulla now only answers a partner's
nulla, so the stress policy forces it far more often but far more narrowly.) `parity.spec.ts` asserts these counts, so a
